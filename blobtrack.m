...
vid.ROIPosition = [0 951 2456 200];
...
% %        break;
  ...
  strain = abs(distance)     ;      %abs for only +values
 out = fscanf(s);
...
if length(data.Y) > 20
   data.Xm(end+1) = mean(data.X(end-20:end));
   data.Ym(end+1) = mean(data.Y(end-20:end));
